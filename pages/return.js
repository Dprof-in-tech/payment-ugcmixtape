...
//   useEffect(() => {
//     const { session_id, p, d, cn, ds } = router.query;
...
export default function Return() {
  ...
  useEffect(() => {
    const { session_id, p, d, cn, couponCode } = router.query;
    ...
    if (session_id) {
      fetch(`/api/checkout_sessions?session_id=${session_id}`, {
        ...
        .then(async (data) => {
          ...
          if (data.status === 'complete') {
            // Payment successful, now update coupon usage
            if (couponCode) {
              try {
                await fetch('/api/updateCoupon', {
                  ...
                  body: JSON.stringify({ couponCode }), // send coupon code and the used amount
                });
              } catch (error) {
